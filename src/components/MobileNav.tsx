            ...
            <SheetClose asChild>
              <nav className="flex h-full flex-col gap-6 text-white-1">
                {sidebarLinks.map(({ route, label, imgURL }) => {
                  ...
                  const routePath = route.startsWith('/profile') ? `/profile/${user?.id}` : route;
                  const isActive = pathname === routePath || pathname.startsWith(`${routePath}/`);

                  return <SheetClose asChild key={routePath}><Link href={routePath} className={cn("flex gap-3 items-center py-4 max-lg:px-4 justify-start", {
